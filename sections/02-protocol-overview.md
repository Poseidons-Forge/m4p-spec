...
- **Data link adaptation.** Configuring modality-specific behaviors (MAC parameters, transmission power) based on operational context and M4P's network topology awareness (see [Section 10.4](#104-data-link-adaptation)). For links that opt into M4P-managed TDMA ([Section 11.10](#1110-tdma-slot-allocation)), the protocol handles TDMA slot allocation automatically; for other links, MAC scheduling remains an application or adapter responsibility. This is deployment-level policy, not per-message adaptation by applications.
...
M4P's DataLink abstraction serves a similar architectural role to BP's Convergence Layer Adapter (CLA) — both decouple the networking layer from link-specific mechanics. M4P's DataLink interface is intentionally narrow: it provides transmission opportunity and payload budget signals to the transport layer. Adaptive link behaviors (MAC configuration, transmission power) are managed by the application layer (see [Section 10.1](#101-datalink-interface)), with one opt-in exception: M4P can manage TDMA slot allocation for links that request it, using the same distributed coordination mechanisms as address assignment (see [Section 11.10](#1110-tdma-slot-allocation)). Existing acoustic communication standards such as JANUS (STANAG 4748) could serve as DataLinks under M4P. M4P does not replace or compete with physical-layer standards — it provides the networking layer above them.
