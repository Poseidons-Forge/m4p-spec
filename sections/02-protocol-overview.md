...
- **Data link adaptation.** Configuring modality-specific behaviors (MAC mode selection, TDMA timing parameters, transmission power) based on operational context and M4P's network topology awareness (see [Section 10.4](#104-data-link-adaptation)). Links that opt into M4P-managed TDMA ([Section 11.10](#1110-tdma-slot-allocation)) receive protocol-managed slot allocation; other links manage their own MAC scheduling. This is deployment-level policy, not per-message adaptation by applications.
...
M4P's DataLink abstraction serves a similar architectural role to BP's Convergence Layer Adapter (CLA) — both decouple the networking layer from link-specific mechanics. M4P's DataLink interface is intentionally narrow: it provides transmission opportunities and payload budgets to the transport layer. In link-managed mode, adaptive MAC behavior remains adapter/application policy. In M4P-managed TDMA mode, participant convergence and slot assignment are protocol-defined via NC messages (see [Section 10.4](#104-data-link-adaptation) and [Section 11.10](#1110-tdma-slot-allocation)). Existing acoustic communication standards such as JANUS (STANAG 4748) could serve as DataLinks under M4P. M4P does not replace or compete with physical-layer standards — it provides the networking layer above them.
