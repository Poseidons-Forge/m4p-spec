...
- **Data link adaptation.** Configuring modality-specific behaviors (MAC mode selection, TDMA timing parameters, transmission power) based on operational context and M4P's network topology awareness (see [Section 10.4](#104-data-link-adaptation)). This is deployment-level policy, not per-message adaptation by applications.
