...
### 13.4 TDMA Departure Signaling

Currently, TDMA participant departure is detected passively via claim expiration after `expiration_interval` ([Section 11.3](#113-claim-expiration-and-renewal)). A departed node's TDMA slot remains occupied until its claim expires, wasting airtime on each cycle. More critically, nodes that hold different `claim_renewal_timestamp` values for the departed peer compute different expiration times, producing divergent schedules during the convergence window. Unlike join-case divergence, which the sender-slot mismatch reconciliation mechanism ([Section 11.10.7](#11107-schedule-divergence-and-reconciliation)) can resolve via NC_TDMA_SCHEDULE exchange, departure-case divergence cannot be reconciled by exchanging participant lists — each node's local expiration computation is authoritative for its own view, and the disagreement persists until all nodes independently expire the departed peer's claim.

A future **NC_TDMA_DEPARTURE** message (Announce propagation) would allow a node to explicitly announce its departure from a TDMA modality, triggering immediate schedule recomputation by peers. This could be sent as part of a graceful shutdown sequence or when a node's link to a TDMA-managed modality is deregistered. Because the departure signal is an explicit event (not a locally-computed expiration), all nodes that receive it can act on it simultaneously, eliminating the convergence gap.

This feature parallels the graceful network departure concept in [Section 13.2](#132-graceful-network-departure) and may be combined with it (e.g., NC_NODE_DEPARTURE triggers removal from all TDMA schedules). Claim expiration remains the correctness backstop for unreachable nodes.
